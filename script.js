...
function faviconFor(u) {
    ...
    return `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" rx="7" fill="#27272a"/><text x="16" y="22" font-family="Inter,sans-serif" font-size="17" font-weight="600" fill="#a1a1aa" text-anchor="middle">${d[0].toUpperCase()}</text></svg>`)}`;
}
function ntFav() {
...
function applyVideo() {
    ...
    if (t?.video) {
        if (v.src !== t.video) { v.src = t.video; v.play().catch(() => {}); }
        document.body.classList.add('has-video-bg');
        v.classList.add('active');
    } else {
        v.classList.remove('active');
        document.body.classList.remove('has-video-bg');
        setTimeout(() => { if (!document.body.classList.contains('has-video-bg')) { v.removeAttribute('src'); v.load(); } }, 900);
    }
}
function applyWallpaper() {
    const t = THEMES.find(t => t.id === state.theme);
    if (t?.video) { document.body.style.backgroundImage = ''; return; }
    document.body.style.backgroundImage = state.wallpaper ? `url(${state.wallpaper})` : '';
}
function createTab(url) {
    const id = state.nextId++;
    const tab = { id, url: url || null, isNewTab: !url, title: 'New Tab', favicon: ntFav(), loading: false };
    state.tabs.push(tab);
    const c = document.getElementById('iframe-container');
    const f = document.createElement('iframe');
    f.id = `iframe-${id}`; f.className = 'tab-iframe hidden';
...
function closeTab(id) {
    ...
    if (i === -1) return;
    const f = document.getElementById(`iframe-${id}`); if (f) f.remove();
...
function renderTabs() {
    ...
    const nb = document.createElement('button'); nb.className = 'new-tab-btn'; nb.title = 'New tab';
    nb.innerHTML = '<i class="fa-solid fa-plus"></i>';
    nb.addEventListener('click', () => createTab()); c.appendChild(nb);
}
...
function navTo(url, id) {
    ...
    tab.url = url; tab.isNewTab = false; tab.title = domain(url) || url; tab.favicon = faviconFor(url); tab.loading = true;
    const f = document.getElementById(`iframe-${id}`);
...
function goHome() {
    const tab = activeTab(); if (!tab) return;
    tab.url = null; tab.isNewTab = true; tab.title = 'New Tab'; tab.favicon = ntFav(); tab.loading = false;
...
function updateBar(tab) {
    const bar = document.getElementById('address-bar'), icon = document.getElementById('address-icon');
    if (tab.isNewTab) { bar.value = ''; bar.placeholder = 'Search or enter URL'; icon.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i>'; }
    else { bar.value = tab.url || ''; icon.innerHTML = tab.url?.startsWith('https://') ? '<i class="fa-solid fa-lock"></i>' : '<i class="fa-solid fa-globe"></i>'; }
}
...
function startLoad() {
    ...
    b.style.transition = 'none'; b.style.width = '0%'; b.style.opacity = '1'; clearTimeout(loadT);
    requestAnimationFrame(() => { b.style.transition = 'width 0.4s ease'; b.style.width = '25%'; loadT = setTimeout(() => { b.style.transition = 'width 2s ease'; b.style.width = '65%'; }, 400); });
}
function finishLoad() {
    clearTimeout(loadT); const b = document.getElementById('loading-bar');
...
function stopLoad() { clearTimeout(loadT); const b = document.getElementById('loading-bar'); b.style.transition = 'opacity 0.2s ease'; b.style.opacity = '0'; }
function onMsg(e) {
    ...
    if (d.type === 'loaded') {
        const t = activeTab(); if (t) {
            t.loading = false; if (d.title) t.title = d.title;
            const f = document.getElementById(`iframe-${t.id}`); if (f) f.classList.remove('loading');
...
function buildSettings() {
    const body = document.getElementById('settings-body'), srvs = allServers();
    body.innerHTML = `<div class="settings-section"><div class="section-label">Proxy Server</div><div class="section-desc">Select a wisp server for routing traffic.</div><div id="srv-list">${srvs.map(s => `<div class="wisp-option${s.url === state.wispUrl ? ' active' : ''}" data-url="${esc(s.url)}"><div class="wisp-radio"><div class="wisp-radio-dot"></div></div><div class="wisp-info"><div class="wisp-name">${esc(s.name)}</div><div class="wisp-url">${esc(s.url)}</div></div><div class="wisp-meta"><span class="ping-badge" data-srv="${esc(s.url)}">—</span><span class="status-dot" data-srv="${esc(s.url)}"></span>${s.url !== DEFAULT_WISP ? `<button class="delete-server" data-del="${esc(s.url)}" title="Remove"><i class="fa-solid fa-trash-can"></i></button>` : ''}</div></div>`).join('')}</div><div class="input-row"><input type="text" id="nw-name" placeholder="Name" autocomplete="off"><input type="text" id="nw-url" placeholder="wss://..." autocomplete="off"><button id="add-wsrv">Add</button></div><button class="ping-all-btn" id="ping-all"><i class="fa-solid fa-signal"></i>&ensp;Ping All</button></div><div class="settings-section"><div class="section-label">Options</div><div class="toggle-row"><span class="toggle-label">Auto-switch on failure</span><div class="toggle-track${state.autoswitch ? ' on' : ''}" id="tg-auto"><div class="toggle-thumb"></div></div></div></div><div class="settings-section"><div class="section-label">Theme</div><div class="section-desc">Video themes include animated backgrounds.</div><div class="theme-grid">${THEMES.map(t => `<button class="theme-swatch${t.id === state.theme ? ' active' : ''}" data-theme="${t.id}"><div class="theme-dot" style="background:${t.color}">${t.video ? '<span class="theme-video-badge"><i class="fa-solid fa-play"></i></span>' : ''}</div><span>${t.name}</span></button>`).join('')}</div></div><div class="settings-section" id="wp-section"><div class="section-label">Wallpaper</div><div class="section-desc">Custom background for non-video themes.</div><div class="wallpaper-row"><input type="text" id="wp-input" placeholder="https://example.com/image.jpg" value="${esc(state.wallpaper)}" autocomplete="off"><button id="set-wp">Set</button></div>${state.wallpaper ? '<button class="remove-wp" id="rm-wp"><i class="fa-solid fa-xmark"></i>&ensp;Remove wallpaper</button>' : ''}</div>`;
    bindSettings();
}
function bindSettings() {
    ...
    document.querySelectorAll('.delete-server').forEach(b => b.addEventListener('click', e => {
        e.stopPropagation(); state.customWisps = state.customWisps.filter(s => s.url !== b.dataset.del);
    ...
    if (ab) ab.addEventListener('click', () => {
        const n = document.getElementById('nw-name').value.trim(), u = document.getElementById('nw-url').value.trim();
        if (!u) return; state.customWisps.push({ name: n || 'Custom', url: u }); saveState(); notifySw(); buildSettings();
    });
...
async function pingAll() {
    ...
    if (navigator.serviceWorker?.controller) { navigator.serviceWorker.controller.postMessage({ type: 'ping' }); }
    else { for (const s of allServers()) { const r = await pingMain(s.url); const dot = document.querySelector(`.status-dot[data-srv="${s.url}"]`); const badge = document.querySelector(`.ping-badge[data-srv="${s.url}"]`); if (dot) { dot.classList.remove('checking'); dot.classList.add(r.ok ? 'ok' : 'fail'); } if (badge) badge.textContent = r.ok ? r.ms + 'ms' : 'fail'; } }
}
function pingMain(url) {
    return new Promise(res => {
        const t0 = Date.now();
        try { const ws = new WebSocket(url); const to = setTimeout(() => { try { ws.close(); } catch {} res({ ok: false }); }, 3000); ws.onopen = () => { clearTimeout(to); res({ ok: true, ms: Date.now() - t0 }); try { ws.close(); } catch {} }; ws.onerror = () => { clearTimeout(to); res({ ok: false }); }; }
        catch { res({ ok: false }); }
    });
}
function notifySw() {
    if (navigator.serviceWorker?.controller) navigator.serviceWorker.controller.postMessage({ type: 'config', wispurl: state.wispUrl, servers: allServers(), autoswitch: state.autoswitch });
}
function handleRedeem() {
    const code = document.getElementById('redeem-input').value.trim().toUpperCase(), st = document.getElementById('redeem-status');
    ...
    if (code.startsWith('AXIS-') && code.length >= 8) {
        ...
        st.className = 'status-ok'; st.textContent = 'Premium unlocked! Reloading...'; setTimeout(() => location.reload(), 1200);
